...
*   **The Pivot:** As progress stalls, the system shifts from **Incremental Tuning** (Safe) toward **Novel Strategy Exploration** (High-Risk).
...
When successful, the agent typically discovers **multiplicative synergies**: for instance, weighting a candidate by the product of an **asymmetric Gaussian top-mass prior** (targeting 162 GeV) and a **tanh-gated eta-correction**. The asymmetry is crucial—the agent "learned" that detector resolution tends to smear energy downward, requiring a wider Gaussian tail on the low-mass side to capture genuine signal. Conversely, when the agent "fails" (resulting in **0.0000 efficiency**), it is often because it proposed a **"Null-Selection Constraint (Physics Veto)"** that was too restrictive—such as a mass window narrower than the fundamental resolution of the calorimeter—leaving zero valid candidates in the event.

To prevent the search from stalling on local optima, the framework employs an **Exponential Refinement Rate Decay**. Initially, the agent acts as a diligent optimizer, spending 80% of its time on **Incremental Tuning** (nudging Gaussian widths by $\pm 1\%$). However, as the `StaleCount` (iterations without a new record) increases, the agent’s "patience" decays. It autonomously pivots its compute budget toward **Novel Strategy Exploration** (spending 90% of its time on "Tabula Rasa" innovation). This allows the system to abandon a plateaued physics hypothesis and "hunt" for entirely new physical discriminants, such as azimuthal symmetry or energy-flow polynomials, ensuring a truthful and exhaustive exploration of the scientific frontier.
...
The term **"Null-Selection Constraint (Physics Veto)"** describes the scenario where the agent’s proposed logic creates a **kinematic impossibility**—such as a mass window narrower than the fundamental detector resolution or a requirement that violates the conservation of momentum. In traditional particle physics, these are known as **Hard Kinematic Cuts** or **Phase Space Restrictions**. By autonomously encountering these "dead ends," the agent mapped the **Forbidden Regions** of the 14-dimensional observable space. This high "failure" rate proves the agent was not playing it safe; it was performing an exhaustive empirical survey of the physical boundaries, eventually learning to avoid unphysical hallucinations and focus its search on the high-purity signal regions that led to the 0.6135 breakthrough.
