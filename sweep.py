...
print("Total truth triplets:", n_total)
...
def evaluate_strategy(gamma, m_target, sigma, w_sigma, min_score, pT_penalty=0.0):
    ...
    for eid, group in events.items():
        # Pre-filter
        group = [row for row in group if row["score"] >= min_score]
        if len(group) == 0:
            continue
...
print("Baseline:", evaluate_strategy(gamma=0.45, m_target=162.0, sigma=22.5, w_sigma=24.0, min_score=0.5))

for min_score in [0.0, 0.2, 0.3, 0.4, 0.5]:
    eff = evaluate_strategy(gamma=0.45, m_target=162.0, sigma=22.5, w_sigma=24.0, min_score=min_score)
    print(f"min_score={min_score} -> {eff:.4f}")
